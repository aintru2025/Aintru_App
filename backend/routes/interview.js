...
const { gpt4oCall, gpt4oSTS, safeAICall, transcribeAudio, analyzeVideo } = require('../config/ai');
const Interview = require('../models/interview');
const InterviewSession = require('../models/interviewSession');
const InterviewReport = require('../models/interviewReport');

// Generate interview question using GPT-4o
router.post('/generate-question', async (req, res) => {
  try {
    ...
    Candidate background: ${candidateProfile?.summary || 'Software engineer with relevant experience'}
...
// Generate AI interviewer voice response using GPT-4o STS
...
// Generate AI response based on candidate answer using GPT-4o
router.post('/ai-response', async (req, res) => {
  try {
    const { transcript, currentQuestion, round, roundType, company, role } = req.body;
    ...
    Return a JSON response with this structure:
    {
      ...
      "aiResponse": "your response as the interviewer"
    }`;
    ...
    const aiResponse = await safeAICall(
      ...
      () => JSON.stringify({
        ...
        nextQuestion: null,
        roundComplete: true,
        aiResponse: "Thank you for that answer. Let's move to the next question."
      }),
...
// Complete interview and generate final report using GPT-4o
router.post('/complete', async (req, res) => {
  try {
    ...
    // Generate comprehensive feedback using GPT-4o
    ...
    - MediaPipe analysis: Eye contact ${mediaPipeData?.eyeContact || 0}/10, Stress ${mediaPipeData?.stress || 0}/10, Posture ${mediaPipeData?.posture || 0}/10
    ...
    Provide a JSON response with:
    {
      ...
      "readinessLevel": "Not Ready|Needs Improvement|Ready|Well Prepared|Excellent",
      "nextSteps": ["step1", "step2"],
      "companiesReady": ["company1", "company2"]
    }`;
    ...
    const feedbackResponse = await safeAICall(
      ...
      () => JSON.stringify({
        ...
        weaknesses: ["Could improve confidence", "Need more specific examples"],
        recommendations: ["Practice more behavioral questions", "Work on confidence"],
        readinessLevel: overallScore >= 8 ? "Ready" : overallScore >= 6 ? "Needs Improvement" : "Not Ready",
