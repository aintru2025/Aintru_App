...
// POST /api/interviewFlow/company-suggestions - Get company suggestions based on query
router.post('/company-suggestions', authenticateToken, async (req, res) => {
  try {
    console.log('🏢 Company suggestions route hit');
    ...
    // Use AI to get company suggestions
...
// POST /api/interviewFlow/role-suggestions - Get role suggestions based on query
router.post('/role-suggestions', authenticateToken, async (req, res) => {
  try {
    console.log('💼 Role suggestions route hit');
    const { query } = req.body;
    ...
    // Use AI to get role suggestions
    const prompt = `Given the search query "${query}", suggest 10-15 relevant job roles/titles that might be available.
    Return only a JSON array of role names, like:
    ["Software Engineer", "Frontend Developer", "Backend Developer", "Full Stack Developer"]

    Focus on roles that are commonly available and relevant to the search term.`;
    ...
    try {
      ...
      suggestions = [
        ...
        'UI/UX Designer', 'Mobile Developer', 'System Administrator', 'Database Administrator',
        'Network Engineer', 'Security Engineer', 'Cloud Engineer', 'Machine Learning Engineer'
      ].filter(role =>
...
// POST /api/interviewFlow/generate-exam-flow - Generate interview flow for exam preparation
router.post('/generate-exam-flow', authenticateToken, async (req, res) => {
  console.log('🎓 Exam flow route hit!');
  ...
  try {
    ...
    // List of known exams for validation
    const knownExams = [
      ...
      'Engineering', 'MBA', 'PhD', 'Research', 'Academic', 'Scholarship'
    ];

    // Check if exam is recognized
    const isKnownExam = knownExams.some(exam => 
    ...
    if (!isKnownExam) {
      ...
      return res.status(400).json({
        ...
        message: 'The exam you entered was not recognized. Please check the spelling and try again with the correct exam name.',
        suggestions: knownExams.filter(exam => 
          exam.toLowerCase().includes(trimmedExamName.toLowerCase().substring(0, 3))
        ).slice(0, 5)
      });
    }

    // Generate exam-specific interview flow using AI
    let interviewFlow;
    try {
      ...
      try {
        ...
        if (!interviewFlow.totalDuration) {
          interviewFlow.totalDuration = interviewFlow.rounds.reduce((total, round) => {
      ...
      // Fallback to predefined exam flows
      interviewFlow = generateFallbackExamFlow(trimmedExamName, trimmedDescription);
    }
    ...
    res.status(500).json({ 
      ...
      message: 'An error occurred while generating the interview flow. Please try again.',
      fallback: {
        ...
        rounds: [
          {
            ...
            focusAreas: ["Core subjects", "Problem solving", "Analytical thinking"]
          },
          {
            round: 2,
...
// Helper function to generate fallback exam flows
    ...
    return {
      ...
      rounds: [
        ...
        },
        {
          ...
          type: "Aptitude",
          duration: "30 mins",
          description: "Verbal ability, numerical ability, and logical reasoning",
          questionType: "aptitude",
          focusAreas: ["Verbal Ability", "Numerical Ability", "Logical Reasoning", "Data Interpretation"]
        },
        {
          round: 3,
          ...
          questionType: "general",
          focusAreas: ["Current Events", "Science & Technology", "History", "Geography"]
        }
      ],
      totalDuration: 105,
      difficulty: 'Hard',
    ...
    return {
      ...
      rounds: [
        ...
        },
        {
          ...
          questionType: "verbal",
          focusAreas: ["Reading Comprehension", "Grammar", "Vocabulary", "Para Jumbles"]
        },
        {
          round: 3,
          ...
          focusAreas: ["Logical Reasoning", "Data Sufficiency", "Puzzles", "Arrangements"]
        }
      ],
      totalDuration: 120,
      difficulty: 'Hard',
      ...
      rounds: [
        {
          ...
          focusAreas: ["History", "Geography", "Polity", "Economics", "Science & Technology"]
        },
        {
          round: 2,
          ...
          focusAreas: ["Essay Writing", "Answer Writing", "Current Affairs", "Optional Subject"]
        },
        {
          round: 3,
          ...
          focusAreas: ["Personality", "Current Affairs", "Hobbies", "Academic Background"]
        }
      ],
      totalDuration: 195,
    ...
    rounds: [
      {
        ...
        questionType: "academic",
        focusAreas: ["Core subjects", "Problem solving", "Conceptual understanding"]
      },
      {
        round: 2,
        ...
        questionType: "aptitude",
        focusAreas: ["Logical reasoning", "Numerical ability", "Verbal ability"]
      },
      {
        round: 3,
        ...
        questionType: "general",
        focusAreas: ["Current events", "General knowledge", "Recent developments"]
      }
    ],
    totalDuration: 90,
    difficulty: 'Medium',
...
// Step 5: Interview Flow Planning (Simplified)
router.post('/generate-interview-flow', authenticateToken, async (req, res) => {
  console.log('🎯 Interview flow route hit!');
  console.log('📝 Request body:', req.body);
  console.log('👤 User from token:', req.user);
  
  try {
    const { company, role, experience, expectedCTC, candidateProfileId } = req.body;
    
    console.log('🔄 Generating interview flow:', { company, role, experience, expectedCTC, candidateProfileId });
    ...
    // Enhanced fallback response with CTC consideration
    const interviewFlow = {
